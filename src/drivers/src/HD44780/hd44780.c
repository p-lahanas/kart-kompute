...
#define Delay_s(__s)                                                    \
  if ((unsigned long)(F_CPU / 1.0 * __s) != F_CPU / 1.0 * __s)          \
    ...
    __builtin_avr_delay_cycles((unsigned long)(F_CPU / 1.0 * __s))
#endif
...
#define lcd_rs_port_set(value) \
  if (value)                   \
    lcd_rs_port_high();        \
  else                         \
    lcd_rs_port_low();
#if RW_LINE_IMPLEMENTED == 1
#define lcd_rw_port_set(value) \
  if (value)                   \
    lcd_rw_port_high();        \
  else                         \
    lcd_rw_port_low();
#endif
#define lcd_db0_port_set(value) \
  if (value)                    \
    lcd_db0_port_high();        \
  else                          \
    lcd_db0_port_low();
#define lcd_db1_port_set(value) \
  if (value)                    \
    lcd_db1_port_high();        \
  else                          \
    lcd_db1_port_low();
#define lcd_db2_port_set(value) \
  if (value)                    \
    lcd_db2_port_high();        \
  else                          \
    lcd_db2_port_low();
#define lcd_db3_port_set(value) \
  if (value)                    \
    lcd_db3_port_high();        \
  else                          \
    lcd_db3_port_low();
#define lcd_db4_port_set(value) \
  if (value)                    \
    lcd_db4_port_high();        \
  else                          \
    lcd_db4_port_low();
#define lcd_db5_port_set(value) \
  if (value)                    \
    lcd_db5_port_high();        \
  else                          \
    lcd_db5_port_low();
#define lcd_db6_port_set(value) \
  if (value)                    \
    lcd_db6_port_high();        \
  else                          \
    lcd_db6_port_low();
#define lcd_db7_port_set(value) \
  if (value)                    \
    lcd_db7_port_high();        \
  else                          \
    lcd_db7_port_low();
...
#define lcd_rs_ddr_set(value) \
  if (value)                  \
    lcd_rs_ddr_high();        \
  else                        \
    lcd_rs_ddr_low();
#if RW_LINE_IMPLEMENTED == 1
#define lcd_rw_ddr_set(value) \
  if (value)                  \
    lcd_rw_ddr_high();        \
  else                        \
    lcd_rw_ddr_low();
#endif
#define lcd_db0_ddr_set(value) \
  if (value)                   \
    lcd_db0_ddr_high();        \
  else                         \
    lcd_db0_ddr_low();
#define lcd_db1_ddr_set(value) \
  if (value)                   \
    lcd_db1_ddr_high();        \
  else                         \
    lcd_db1_ddr_low();
#define lcd_db2_ddr_set(value) \
  if (value)                   \
    lcd_db2_ddr_high();        \
  else                         \
    lcd_db2_ddr_low();
#define lcd_db3_ddr_set(value) \
  if (value)                   \
    lcd_db3_ddr_high();        \
  else                         \
    lcd_db3_ddr_low();
#define lcd_db4_ddr_set(value) \
  if (value)                   \
    lcd_db4_ddr_high();        \
  else                         \
    lcd_db4_ddr_low();
#define lcd_db5_ddr_set(value) \
  if (value)                   \
    lcd_db5_ddr_high();        \
  else                         \
    lcd_db5_ddr_low();
#define lcd_db6_ddr_set(value) \
  if (value)                   \
    lcd_db6_ddr_high();        \
  else                         \
    lcd_db6_ddr_low();
#define lcd_db7_ddr_set(value) \
  if (value)                   \
    lcd_db7_ddr_high();        \
  else                         \
    lcd_db7_ddr_low();
